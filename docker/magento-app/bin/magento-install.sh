...
# NOTE: the config hash this install just recorded does NOT match what the
# long-lived pods will run — setup:install rewrote app/etc/config.php in this
# pod only. Reconciling it has to happen in a *different* pod, which is what the
# config-sync Job does; see charts/magento/templates/config-sync-job.yaml.
echo "[install] done."
